...
- **process-todo** : lors du traitement des actions complexes (`other` dans `do-decide`, `do-consult-and-decide`, `do-other`, `do-self`), ce skill est utilisé pour le lookup flow complet (contextualisation) et pour la mise à jour de la mémoire après chaque finalisation (nouveaux sujets, collaborateurs, préférences).
