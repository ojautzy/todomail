...
```bash
python3 -c "from lib import state, fs_utils, rag_cache, error_modes; print('OK')"
```
...
Fichier : `${CLAUDE_PLUGIN_DATA}/state.json` (fallback : `.plugin-data/state.json`)

| Fonction | Description |
|----------|-------------|
| `load_state()` | Charge ou cree le state avec schema par defaut |
| `save_state(state)` | Ecriture atomique (.tmp + os.replace) |
...
| `acquire_lock(name)` | Pose un verrou consultatif |
| `release_lock()` | Libere le verrou |
...
Classe `RagCache` : cache cle-valeur en memoire pour eviter les appels MCP redondants.
